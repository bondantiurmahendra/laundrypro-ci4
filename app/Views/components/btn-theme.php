<!-- <div class="relative flex w-full max-w-xs flex-col gap-1 p-2 text-on-surface dark:text-on-surface-dark">
  <label for="os" class="w-fit pl-0.5 text-sm">Variant</label>
  <select id="os" name="os" x-model="theme" @change="setTheme($event.target.value)"
...
<div x-data class="relative flex w-full max-w-xs flex-col gap-1 text-on-surface dark:text-on-surface-dark">
  <button
    class="btn btn-sm btn-primary w-full appearance-none rounded-radius border border-outline bg-surface-alt px-4 py-2 text-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary disabled:cursor-not-allowed disabled:opacity-75 dark:border-outline-dark dark:bg-surface-dark-alt/50 dark:focus-visible:outline-primary-dark flex items-center justify-between "
    @click="() => {
      toggleDark()
      
    console.log(getComputedStyle(document.documentElement).getPropertyValue('--grid-color'))
    }">
    <h3 class="w-fit pl-0.5 text-sm">Tema Aktif</h3>
...
<div class="relative flex flex-col w-full">
  <h1 class="mb-1 flex items-center pl-0.5 text-sm text-onSurface dark:text-onSurfaceDark">
    Variant Tema
  </h1>
  <div
    class="hide-scrollbar h-full flex w-full flex-col gap-1 overflow-auto border-outline bg-surfaceAlt p-1 text-sm shadow-[0_3px_10px_rgb(0,0,0,0.1)] dark:border-outlineDark dark:bg-surfaceDarkAlt rounded-xl border-none"
    x-data="{ themes: [] }" x-init="themes = window.themeList" @click.outside="showThemes = false">
    <template x-for="t in themes" :key="t.name">
      <button
        class="relative flex w-full cursor-pointer items-center justify-between gap-4 border border-outline bg-surface/50 p-2 transition hover:bg-surface/50 dark:border-outlineDark dark:bg-surface-dark-alt/50 dark:hover:bg-surface-dark-alt/50 rounded-md"
        :aria-label="t.label" :aria-pressed="theme === t.name" @click="setTheme(t.name)">
        <!-- icon dan label -->
        <div class="flex items-center gap-2" aria-hidden="true">
          <div x-show="theme !== t.name"
          ...
          <svg x-show="theme === t.name" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
        ...
        <!-- preview palette -->
        <div class="flex h-full items-center gap-1 p-2" :class="[t.font, t.rounded]">
          <template x-for="(color, i) in t.preview" :key="i">
            <div class="flex items-center p-1.5" :class="color"></div>
          </template>
        </div>
      </button>
    </template>
  ...
  <!-- <div
    class="hide-scrollbar h-full flex w-full flex-col gap-1 overflow-auto border-outline bg-surfaceAlt p-1 text-sm shadow-[0_3px_10px_rgb(0,0,0,0.1)] dark:border-outlineDark dark:bg-surfaceDarkAlt rounded-xl border-none"
    x-transition="" @click.outside="showThemes = false" :class="['rounded']">
    <button aria-label="Arctic" :aria-pressed="theme === 'arctic'"
      class="relative flex w-full cursor-pointer items-center justify-between gap-4 border border-outline bg-surface/50 p-2 transition hover:bg-surface-alt/50 dark:border-outlineDark dark:bg-surface-dark-alt dark:hover:bg-surface-dark-alt/50 rounded-xl"
      :class="[ 'rounded' ]" x-on:click="setTheme('arctic')" aria-pressed="false">
      <div class="flex items-center gap-2" aria-hidden="true">
